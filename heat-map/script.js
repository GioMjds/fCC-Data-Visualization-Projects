...
function callback(data) {
  ...
  data.monthlyletiance.forEach(function (val) {
    val.month -= 1;
  });
  ...
  heading
    ...
    .attr("id", "description")
    .html(
      data.monthlyletiance[0].year +
        " - " +
        data.monthlyletiance[data.monthlyletiance.length - 1].year +
        ": base temperature " +
  ...
  let fontSize = 16;
  let width = 5 * Math.ceil(data.monthlyletiance.length / 12);
  let height = 33 * 12;
  let padding = {
    left: 9 * fontSize,
    right: 9 * fontSize,
  ...
  let xScale = d3
    .scaleBand()
    .domain(
      data.monthlyletiance.map(function (val) {
        return val.year;
      })
    )
    .range([0, width])
  ...
  let letiance = data.monthlyletiance.map(function (val) {
    return val.letiance;
  });
  let minTemp = data.baseTemperature + Math.min.apply(null, letiance);
  let maxTemp = data.baseTemperature + Math.max.apply(null, letiance);
  ...
  svg
    ...
    .attr("data-temp", function (d) {
      return data.baseTemperature + d.letiance;
    })
    ...
    .attr("fill", function (d) {
      return legendThreshold(data.baseTemperature + d.letiance);
    })
    .on("mouseover", function (event, d) {
      ...
      let str =
        ...
        d3.format(".1f")(data.baseTemperature + d.letiance) +
        "&#8451;" +
        "</span>" +
        "<br />" +
        "<span class='letiance'>" +
        d3.format("+.1f")(d.letiance) +
        "&#8451;" +
        "</span>";
